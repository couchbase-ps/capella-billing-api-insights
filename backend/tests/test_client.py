...
def make_client(clock: FakeClock, **kwargs: object) -> CapellaClient:
    ...
    return CapellaClient(
        "secret-key", BASE, limiter=limiter, retry=retry, sleep=clock.sleep, org_id=ORG, **kwargs
    )
...
async def test_429_then_200_honours_retry_after(router: respx.MockRouter) -> None:
    clock = FakeClock()
    route = router.get("/v4/organizations").mock(
        side_effect=[
            httpx.Response(429, headers={"Retry-After": "3"}, json={"code": 1004, "message": "slow"}),
...
@respx.mock(base_url=BASE)
async def test_5xx_is_retried_with_backoff(router: respx.MockRouter) -> None:
    clock = FakeClock()
...
@respx.mock(base_url=BASE)
async def test_gives_up_after_five_attempts(router: respx.MockRouter) -> None:
    clock = FakeClock()
    route = router.get("/v4/organizations").mock(return_value=httpx.Response(500, json={"message": "boom"}))
    async with make_client(clock) as client:
...
@respx.mock(base_url=BASE)
async def test_401_raises_api_error_without_retry(router: respx.MockRouter) -> None:
    clock = FakeClock()
    route = router.get("/v4/organizations").mock(
        return_value=httpx.Response(
            401, json={"httpStatusCode": 401, "code": 1001, "message": "bad key", "hint": "check it"}
        )
    )
    async with make_client(clock) as client:
        with pytest.raises(CapellaAPIError) as info:
...
@respx.mock(base_url=BASE)
async def test_bearer_header_is_sent(router: respx.MockRouter) -> None:
    clock = FakeClock()
    route = router.get("/v4/organizations").mock(return_value=httpx.Response(200, json={"data": []}))
    async with make_client(clock) as client:
        await client.list_organizations()
...
@respx.mock(base_url=BASE)
async def test_walk_pages_follows_cursor(router: respx.MockRouter) -> None:
    clock = FakeClock()
    ...
    def respond(request: httpx.Request) -> httpx.Response:
        ...
        if page == 1:
            return httpx.Response(
                200,
                json={"data": [{"id": "p1", "name": "one"}], "cursor": {"pages": {"page": 1, "next": 2, "last": 2}}},
            )
        return httpx.Response(200, json={"data": [{"id": "p2", "name": "two"}], "cursor": {"pages": {"page": 2, "last": 2}}})
...
@respx.mock(base_url=BASE)
async def test_categorized_billing_request_body(router: respx.MockRouter) -> None:
    clock = FakeClock()
    payload = {
        "data": {
            "billingCurrency": "USD",
            "periods": [
                {
                    ...
                    "endDate": "2026-08-01",
                    "categories": [
                        {"category": "operationalComputeAndStorage", "creditSpend": 1.5, "currencySpend": None, "contributionPercent": 100}
                    ],
                    "totalCreditSpend": 1.5,
                    "totalCurrencySpend": None,
                }
            ],
            "total": {"startDate": "2026-08-01", "endDate": "2026-08-01", "categories": [], "totalCreditSpend": 1.5},
        }
    }
    route = router.post(f"/v4/organizations/{ORG}/billing").mock(return_value=httpx.Response(200, json=payload))
    async with make_client(clock) as client:
        billing = await client.categorized_billing(date(2026, 8, 1), date(2026, 8, 1), instance_ids=["c1"])
    import json

    body = json.loads(route.calls.last.request.content)
    assert body == {"startDate": "2026-08-01", "endDate": "2026-08-01", "filters": {"instanceIds": ["c1"]}}
