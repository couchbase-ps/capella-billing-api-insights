...
`GET /api/sync/status` → `{"running":false,"runs":[{"id","startedAt","finishedAt","status","requestsMade","error","detail":{"clustersSynced":5,"bucketsSkipped":2,"endpointsSkipped":1,"appServicesSynced":1,"analyticsClustersSynced":1,"billingWindows":6,"failures":[{"scope":"cluster","instanceId":"..","message":".."}]}}]}` (last 10 runs, newest first)
